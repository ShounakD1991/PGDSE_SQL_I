...
SELECT 
    product, SUM(quantity) AS Total_Quantity, month
FROM
    bank_inventory_pricing
...
SELECT 
    product,
    ROUND(MAX(Estimated_sale_price - Purchase_cost),
            2) AS Profit
FROM
    bank_inventory_pricing
GROUP BY product
...
FROM
    bank_branch_pl
GROUP BY banker , branch
HAVING (Estimated_profit > 20000000);
/* Pre-requisite */
...
SELECT 
    product, month, SUM(quantity) AS Total_Quantity
FROM
    bank_inventory_pricing
GROUP BY product
...
FROM
    bank_branch_pl
GROUP BY branch
HAVING Total_Profit > Estimated_profit;
# Question 8:
...
SELECT 
    ...
    SUM(cost) AS Total_Cost,
    SUM(revenue) AS Total_Revenue,
    SUM(cost - revenue) AS Total_Loss
FROM
    bank_branch_pl
GROUP BY branch
HAVING Total_Loss > 0;
...
SELECT 
    branch, product, MIN(revenue - cost) AS Least_Profit
FROM
    bank_branch_pl
GROUP BY branch , product
HAVING least_profit > 0
...
SELECT 
    *, SUM(revenue - cost) AS Total_Profit, estimated_profit
FROM
    bank_branch_pl
GROUP BY branch , product
HAVING total_profit > estimated_profit
...
FROM
    bank_branch_pl
GROUP BY branch , product
HAVING total_profit > estimated_profit*0.9
    AND total_profit > 0;
# Question 13:
...
SELECT 
    *,
    SUM(revenue - 0.7 * cost) AS Total_Profit,
    estimated_profit
FROM
    bank_branch_pl
GROUP BY branch , product
